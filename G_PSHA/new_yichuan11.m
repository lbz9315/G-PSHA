...
function [sum_Length,shortest_Route,a4,disHistoryUAV,distHistory] = new_yichuan11(xy,dmat,nSalesmen,minTour,popSize,numIter,showProg,showResult,i,M,la)
...
HisdisUAV=zeros(popSize,nSalesmen);  %记录每一个个体的每一个粒子的解
...
%         end
        ...
        for s = 1:nSalesmen
            ZQ(a2{s},M)
          if ZQ(a2{s},M)<= la 
              while ZQ(a2{s},M)<la && s<nSalesmen && ~isempty(a2{s+1})    %&& ZQ([a2{s};a2{s+1}(1,:)],M)<=la
                 % s
                  if ZQ([a2{s};a2{s+1}(1,:)],M)< la
                      a2{s}=[a2{s};a2{s+1}(1,:)];
                      a2{s+1}(1,:)=[];
              ...
              while ZQ(a2{s},M)>la && s<nSalesmen
                  
                      lastRowOfA2_1 = a2{s}(end,:);
                      a2{s}(end,:)=[];
                      a2{s+1} = [lastRowOfA2_1; a2{s+1}];
                      pBreak(s)=pBreak(s)-1; 
                      
                      a3{s+1}=[a3{s}(:,end),a3{s+1}];
                      a3{s}(:,end)=[];
...
%             shortest_Route={};
        ...
        %矫正不起飞无人机的飞行距离
              ...
              length=[length, shortest_Length];  %各无人机的最短距离
              sRoute{s}=shortest_Route1;  %各无人机的最短路径（序列）
